...
def p51():
    # _indexes = where the replicated digits will be.
    # result = False if None,
    def get_smallest_prime_with_8prime_family(num_of_digits, indices):
        curr_min = None
        for p in product((str(i) for i in range(10)), repeat=num_of_digits - len(indices)):
            p_str = ''
            for i in range(num_of_digits):
                # p_str will be 'd2d3d3' for _indexes=(0,2,4), p=(2,3,3) and _num_of_digits=6
                if i in indices:
                    p_str += 'd'
                else:
                    p_str += p[0]
                    p = p[1:]
            start = 1 if 0 in indices else 0
        ...
        # print(''.join(['*' if i in _indexes else '_' for i in range(_num_of_digits)]))

    _num_of_digits = 1
    while True:
        for num_of_indexes in range(3, _num_of_digits, 3):
            # _indexes are good only if the number of them is divisible by 3
            # (so that it may be that None of the tested numbers will be divided by 3),
            # and should leave the Least-Significant-Digit not in _indexes, because of even numbers...
            for _indexes in combinations(range(_num_of_digits - 1), num_of_indexes):
                res = get_smallest_prime_with_8prime_family(_num_of_digits, _indexes)
                if res:
                    return res
        _num_of_digits += 1
...
def p54(path='Data/p054_poker.txt'):
    # curr[0] = (High Card, One Pair, Two Pairs, Three of a Kind, Straight, Flush, Full House,
    ...
    def get_points(hand):
        ...
        if 2 in hist:
            left, right = hist.index(2), len(hist) - 1 - hist[::-1].index(2)
            if left != right:
                return 2, max(left, right), min(left, right), sorted(nums)[::-1]
            else:
                return 1, left, sorted(nums)[::-1]
...
#                 p3 = primes[i3]
#                 if (p1 + p2 + p3) * 5 / 3 >= high:
#                     break
#                 for i4 in range(i3 + 1, n):
#                     p4 = primes[i4]
#                     if (p1 + p2 + p3) * 5 / 4 >= high:
#                         break
